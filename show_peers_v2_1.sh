...
__collectHostData () {
    ...
    for i in $*
    do
        ssh -o ConnectTimeout=1 $i 'monit summary 2>/dev/null ; ps -ef 2>/dev/null' >$tmpDir/${i}/collectHostData_out.txt 2>$tmpDir/${i}/collectHostData_err.txt 
    done
...
__getPortData () {
    ...
    do 
        ...
        for port in $(grep '^Process.*qns-[2-9]' $tmpDir/${lb}/collectHostData_out.txt 2>/dev/null| sed 's/.*qns-\([2-9]\).*/909\1/' 2>/dev/null| sort -n 2>/dev/null| paste -s -)
        do
...
__getInstanceId () {
    ...
    do
        for port in $(cat $tmpDir/${lb}/getPortData_out.txt)
        do
       grep "\-console ${port}" $tmpDir/${lb}/collectHostData_out.txt 2>/dev/null |  sed 's/.*adhop.run.instanceId=\([^ ]\+\) .*/\1/'  2>/dev/null >$tmpDir/${lb}/collectPeerData-id_${port}.txt &
        done
    done
}
